...
namespace lua_ui_bridge
{
    namespace
    {
        ...
        bool has_checkbox_anchor(const tab_state& tab, const std::string& key)
        {
            const auto existing = tab.widget_index_by_key.find(key);
            if (existing != tab.widget_index_by_key.end())
            {
                const std::size_t index = existing->second;
                return index > 0 && tab.widgets[index - 1].type == widget_type::checkbox;
            }
            ...
            return tab.widgets.back().type == widget_type::checkbox;
        }
    }
    ...
    bool add_keybind_widget(int tab_id, const std::string& key, const std::string& label, int default_key, int default_mode)
    {
        ...
        if (!has_checkbox_anchor(*tab, key))
        {
            return false;
        }
