...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    const float max_picker_size = g.FontSize * 9.0f;
    ...
    float square_sz = GetFrameHeight() * 0.75f;
    float bars_width = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = ImMax(bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    sv_picker_size = ImMin(sv_picker_size, max_picker_size);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2); // Sneakily prevent the indicator to stick out too much
        ...
        const float hue_marker_half_thickness = 1.5f;
        const ImU32 marker_fill_col = IM_COL32(255, 255, 255, 255);
        const ImU32 marker_outline_col = IM_COL32(0, 0, 0, 255);
        ImVec2 hue_marker_min = ImFloor(ImVec2(bar0_pos_x - 1.0f, bar0_line_y - hue_marker_half_thickness));
        ImVec2 hue_marker_max(hue_marker_min.x + bars_width + 1.0f, hue_marker_min.y + hue_marker_half_thickness * 2.0f);
        draw_list->AddRectFilled(hue_marker_min, hue_marker_max, marker_fill_col);
        draw_list->AddLine(ImVec2(hue_marker_min.x, hue_marker_min.y), ImVec2(hue_marker_max.x, hue_marker_min.y), marker_outline_col);
        draw_list->AddLine(ImVec2(hue_marker_min.x, hue_marker_max.y), ImVec2(hue_marker_max.x, hue_marker_max.y), marker_outline_col);
        draw_list->AddLine(ImVec2(hue_marker_min.x, hue_marker_min.y), ImVec2(hue_marker_min.x, hue_marker_max.y), marker_outline_col);
        draw_list->AddLine(ImVec2(hue_marker_max.x, hue_marker_min.y), ImVec2(hue_marker_max.x, hue_marker_max.y), marker_outline_col);
    }

    // Render cursor/preview square (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    const float selector_half = 3.0f;
    const float selector_size = selector_half * 2.0f;
    ImVec2 selector_min = ImFloor(ImVec2(sv_cursor_pos.x - selector_half, sv_cursor_pos.y - selector_half));
    ImVec2 selector_max(selector_min.x + selector_size, selector_min.y + selector_size);
    draw_list->AddRectFilled(selector_min, selector_max, col_white);
    const ImU32 selector_outline = IM_COL32(0, 0, 0, 255);
    draw_list->AddRect(selector_min, selector_max, selector_outline);
    ...
    if (alpha_bar)
    {
        ...
        const float alpha_marker_half_thickness = 1.5f;
        const ImU32 marker_fill_col = IM_COL32(255, 255, 255, 255);
        const ImU32 marker_outline_col = IM_COL32(0, 0, 0, 255);
        ImVec2 alpha_marker_min = ImFloor(ImVec2(bar1_pos_x - 1.0f, bar1_line_y - alpha_marker_half_thickness));
        ImVec2 alpha_marker_max(alpha_marker_min.x + bars_width + 1.0f, alpha_marker_min.y + alpha_marker_half_thickness * 2.0f);
        draw_list->AddRectFilled(alpha_marker_min, alpha_marker_max, marker_fill_col);
        draw_list->AddLine(ImVec2(alpha_marker_min.x, alpha_marker_min.y), ImVec2(alpha_marker_max.x, alpha_marker_min.y), marker_outline_col);
        draw_list->AddLine(ImVec2(alpha_marker_min.x, alpha_marker_max.y), ImVec2(alpha_marker_max.x, alpha_marker_max.y), marker_outline_col);
        draw_list->AddLine(ImVec2(alpha_marker_min.x, alpha_marker_min.y), ImVec2(alpha_marker_min.x, alpha_marker_max.y), marker_outline_col);
        draw_list->AddLine(ImVec2(alpha_marker_max.x, alpha_marker_min.y), ImVec2(alpha_marker_max.x, alpha_marker_max.y), marker_outline_col);
    }
